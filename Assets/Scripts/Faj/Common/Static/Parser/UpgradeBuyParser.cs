...
using Faj.Common.Static.Upgrade.Buy.Collection;
...
using Faj.Common.Static.Upgrade.Buy.Collection.Item;

namespace Faj.Common.Static.Parser
{
	class UpgradeBuyParser : IStaticParser
    {
        public IStaticCollection Parse(XDocument document)
        {
            
            var collection = new UpgradeBuyCollection();
        ...
        IUpgradeBuyItem ParseItem(XElement element)
        {
            ...
            if (element.Element("checkstart") != null)
            {
                ...
                foreach (var checkStartElement in element.Element("checkstart").Elements())
                {
                    ...
                    if (checkStartElement.Name == "upgrades")
                    {
                        var idElement = checkStartElement.Element("level");
                        var intCondition = new IntCondition((int)idElement);
                        var upgradesModule = new UpgradesModule(intCondition);
                        checkStart.Add(upgradesModule);
                    }
            ...
            if (element.Element("award") != null)
            {
                ...
                {
                    if (awardElement.Name == "upgrades")
                    {
                        var idElement = awardElement.Element("level");
                        level = (int)idElement;
                        var intCondition = new IntCondition(level);

                        var upgradesModule = new UpgradesModule(intCondition);
                        award.Add(upgradesModule);
                    }
                }
            }

            var item = new UpgradeBuyItem(id, checkStart, null, pay, award, null, type, level);
